...
public class MazeProblem {
    ...
    Set<MazeState> GOAL_STATES = new HashSet<>();
    Set<MazeState> KEY_STATES = new HashSet<>(); 
    ...
    MazeProblem (String[] maze) {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                case 'K':
                	KEY_STATES.add(new MazeState(col, row)); break;
                case '.':
                case 'X':
                case 'M':
                    break;
    ...
    /**
     * Returns whether or not a state is the key state
     * @param state A MazeState (col, row) to test
    ...
    /**
     * Returns the cost of moving onto a MazeState
     * @param state The maze state that we return the cost of
     * @return The cost that it takes to move onto this state
     */
